// script.js - Полностью обновленный скрипт с календарем и занятыми слотами
...
function updateTimeSlots() {
    ...
    const occupiedSlots = getOccupiedTimeSlots(selectedTeacher, selectedDate);
...
function getOccupiedTimeSlots(teacher, date) {
...
function handleBookingSubmit(event) {
    ...
    const occupiedSlots = getOccupiedTimeSlots(teacherSelect.value, dateInput.value);
    ...
    // Сохраняем запись в локальное хранилище
...
// Функция инициализации админ-панели (остается без изменений)
...
function handleAdminLogin(event) {
    ...
    // Проверяем учетные данные (в реальном приложении это делается на сервере)
    if (username === 'admin' && password === 'admin123') {
        ...
        console.log('Авторизация не удалась: неверные учетные данные');
...
function loadBookings() {
    ...
    // Получаем записи из localStorage
    let bookings = JSON.parse(localStorage.getItem('teacherBookings')) || [];
    console.log('Загружено записей из localStorage:', bookings.length);
...
function handleDeleteBooking(event) {
    ...
    if (confirm(`Вы уверены, что хотите удалить запись #${bookingId}?`)) {
        ...
        // Показываем уведомление
        if (bookingToDelete) {
            showNotification(`Запись #${bookingId} (${bookingToDelete.teacherName}, ${bookingToDelete.parentName}) удалена`, 'info');
        }
    }
}
...
// Основная функция экспорта в Excel (остается без изменений)
...
// Функция создания листа со статистикой по субботам (остается без изменений)
function createSaturdayStatsSheet(wb, bookings) {
    ...
    bookings.forEach(booking => {
        ...
        statsBySaturday[saturdayDate].teachers.add(booking.teacherName);
    ...
    const allTeachers = new Set(bookings.map(b => b.teacherName)).size;
...
// Функция показа уведомления (остается без изменений)
...
// Функция создания демо-данных (для тестирования)
function initDemoData() {
    // Проверяем, есть ли уже данные
    const bookings = JSON.parse(localStorage.getItem('teacherBookings')) || [];
    
    if (bookings.length === 0) {
        console.log('Создание демо-данных (только субботы)...');
        ...
        // Форматируем дату
        const saturdayDateStr = nextSaturday.toISOString().split('T')[0];
        
        // Создаем несколько тестовых записей на субботу
        const demoBookings = [
            {
                id: Date.now() + 1,
                ...
                comment: "Обсуждение успеваемости по математике",
                status: "новая",
                createdAt: new Date().toISOString()
            },
            {
                id: Date.now() + 2,
                teacher: "Петров Алексей Владимирович|2010",
                teacherName: "Петров Алексей Владимирович",
                teacherRoom: "2010",
                date: saturdayDateStr,
        ...
        localStorage.setItem('teacherBookings', JSON.stringify(demoBookings));
        console.log('Демо-данные созданы (только субботы):', demoBookings.length, 'записей');
        
        // Показываем уведомление
        showNotification('Демо-данные созданы для тестирования системы', 'info');
    }
}

// Раскомментируйте для создания демо-данных при загрузке (для тестирования)
// initDemoData();